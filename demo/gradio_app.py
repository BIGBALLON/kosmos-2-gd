...
Batch = namedtuple(
    "Batch", "ids src_tokens src_lengths constraints img_src_tokens img_gpt_input_mask"
)
    ...
    def convert_one_line(input_str):
        ...
        for i, segment in enumerate(segments):
            if segment.startswith("[image]"):
                ...
                token.extend(
                    [boi_id] + list(range(4, image_feature_length + 4)) + [eoi_id]
                )

                img_gpt_input_mask.extend([0] + [1] * image_feature_length + [0])
            else:
                special_tokens = [
                    self.source_dictionary[idx]
                    for idx in range(
...
def make_batches(lines, cfg, task, max_positions, encode_fn):
    ...
    (
        ...
        img_src_tokens,
        img_gpt_input_mask,
    ) = get_interactive_tokens_and_lengths(task, lines, encode_fn, tokenizer)
...
def main(cfg: FairseqConfig):
    ...
    def generate_predictions(
        image_input, text_input, do_sample, sampling_topp, sampling_temperature
    ):
        if do_sample:
        ...
        with gr.Row():
            with gr.Column():
                ...
                do_sample = gr.Checkbox(
                    ...
                    info="(Please enable it before adjusting sampling parameters below)",
                    value=False,
                )
                with gr.Accordion(
                    "Sampling parameters", open=False
                ) as sampling_parameters:
                    sampling_topp = gr.Slider(
                        minimum=0.1,
                        maximum=1,
                        step=0.01,
                        value=0.9,
        ...
        with gr.Row():
            with gr.Column():
                gr.Examples(
                    examples=[
                        [
                            "demo/images/man_ball.png",
                            "<grounding> Describe this image in detail:",
                            False,
                        ],
                        [
                            "demo/images/man_ball.png",
                            "<grounding> Question: Are there any people in the image, if so, detect them.  Answer:",
                            False,
                        ],
                        [
                            "demo/images/two_dogs.jpg",
                            "<grounding> Describe this image in detail:",
                            False,
                        ],
                        [
                            "demo/images/two_dogs.jpg",
                            "<grounding> Are there any dogs in the image? Answer: ",
                            False,
                        ],
                    ],
            ...
            with gr.Column():
                gr.Examples(
                    examples=[
                        [
                            "demo/images/quadrocopter.jpg",
                            "<grounding> Describe this image in detail:",
                            True,
                        ],
                        [
                            "demo/images/quadrocopter.jpg",
                            "<grounding> Question: Are there any people in the image, if so, only detect them. Answer:",
                            False,
                        ],
                        [
                            "demo/images/six_planes.png",
                            "<grounding> Describe this image in detail:",
                            True,
                        ],
                        [
                            "demo/images/six_planes.png",
                            "<grounding> Question: How many planes are there in the image, please detect them separately. Answer:",
                            True,
                        ],
                    ],
                    inputs=[image_input, text_input, do_sample],
...
def mark_words(text, phrases):
    ...
    word_indices = [
        match.start() for match in re.finditer(r'\b\w+\b|[.,;?!:()"“”‘’\']', text)
    ]
